...
class ClockInForm extends StatefulWidget {
  ...
  ClockInForm(this.success, this.failed);
...
class _ClockInFormState extends State<ClockInForm> {
  ...
  void requestNetwork() async {
    ...
    Map<String, String> headers = {
      "Authorization": token,
      "ClockInLat": currentLatitude.toString(),
      "ClockInLng": currentLongitude.toString(),
      "ClockInNote": _noteController.text,
    };

    var response = await clockInService(headers);

    if (response.statusCode == 200) {
      Navigator.of(context).pop();
      Navigator.of(context).pop();
      widget.success(0);
      Toast_SUCCESS(context, "Congratulations", "Clocked-in Successfully");
    } else{
      Navigator.of(context).pop();
      Navigator.of(context).pop();
      widget.failed(0);
